                ...
                <tbody>
                    @foreach($prices as $key => $price)
                        <tr data-entry-id="{{ $price->id }}">
                            ...
                            <td>
                                @foreach($price->amenities2 as $key => $item)
                                    <span class="badge badge-info">{{ $item->name }}</span>
                                @endforeach
                            </td>
                            <td>
                                @can('price2_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.prices2.show', $price->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('price2_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.prices2.edit', $price->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('price2_delete')
                                    <form action="{{ route('admin.prices2.destroy', $price->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
